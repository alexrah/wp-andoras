...
$options_slider[] = array( "name" => "Background Color",
					"desc" => "",
					"id" => "bg_color",
					"std" => "#dcdcdc",
					"type" => "color"); 
					
$options_slider[] = array( "name" => "Image description ",
					"desc" => "the description that will be shown on the slider",
					"id" => "description",
					"std" => "",
					"type" => "textarea"); 
					
$options_slider[] = array( "name" => "Link",
					"id" => "link",
					"std" => "",
					"type" => "text"); 
    ...
    <script type="text/javascript" language="javascript">
		jQuery(document).ready(function(){
			...
			foreach($options_slider as $option){ 
...
function meta_options_slider($options_slider) {
	
	create_js_slider();
	
	global $options_slider,$post;
	
	$output= '<input type="hidden" name="mytheme_meta_box_nonce" value="'. wp_create_nonce(basename(__FILE__)). '" />';
        
    $counter = 0;
	$menu = '';
	foreach ($options_slider as $value) {
...
function create_meta_box_slider() {
	...
	add_meta_box( 'new-meta-boxe', 'Cora Slider Settings', 'meta_options_slider', 'slideshow', 'normal', 'high' );
	}
}

function mytheme_save_data_slider($post_id) {
    global $options_slider;
	global $post;
	
	if(get_post_type( $post )=='slideshow'){
...
add_action('save_post', 'mytheme_save_data_slider');
