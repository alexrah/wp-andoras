...
function optionsframework_add_admin() {
	...
	$of_page = add_menu_page( $themename, $themename, 'edit_theme_options', 'optionsframework', 'optionsframework_options_page');
...
function of_load_only() {
	...
	//wp_register_script('jquery-input-mask', OF_DIRECTORY.'/functions/js/jquery.maskedinput-1.2.2.js', array( 'jquery' ));
		wp_register_script('jquery-input-mask', OF_DIRECTORY.'/admin/js/jquery.maskedinput-1.2.2.js');
	...
	function of_admin_head() { 
		...
		echo '<link rel="stylesheet" type="text/css" href="'.OF_DIRECTORY.'/admin/admin-style.css" media="screen" />';
		
		 // COLOR Picker ?>
<link rel="stylesheet" media="screen" type="text/css" href="<?php echo OF_DIRECTORY; ?>/admin/css/colorpicker.css" />
<script type="text/javascript" src="<?php echo OF_DIRECTORY; ?>/admin/js/colorpicker.js"></script>
...
<?php
		//AJAX Upload
		?>
<script type="text/javascript" src="<?php echo OF_DIRECTORY; ?>/admin/js/ajaxupload.js"></script>
...
function optionsframework_machine($options) {
	...
	foreach ($options as $value) {
	    ...
	    case "range":
			...
			$output .= '<input style="display:inline; width:40px;" type="range" name="' . $value['id'] . '" value="';
...
//			$output.= '<tr><th scope="row"><h4>' . $value['name'] . '</h4></th><td>';

			if (isset($value['function']) && function_exists($value['function'])) {
				$output.= theme_add_sidebar_option($value);
			} else {
				$output.= $value['html'];
			}
			$output.= '</td></tr>';
...
if (! function_exists("theme_add_sidebar_option")) {
	function theme_add_sidebar_option($value) {
        $default=get_option('sidebars');
		if(!empty($default)){
...
';
		...
		$output.= '<input type="text" id="add_sidebar" style="width:200px" name="add_sidebar"/>';
		
		$output.='<input id="create_sidebar" class="button" type="button" style="width:100px;" value="create"/>';

		if(!empty($sidebars)){
			$output.='<h3 class="heading">Below are the '.count($sidebars).' sidebars you have created</h3>';
			
			foreach($sidebars as $sidebar){
				$output.= '<div class="sidebar-item"><span>'.$sidebar.'</span><input type="hidden" class="sidebar-item-value" value="'.$sidebar.'"/><input type="button" class="button" style="width:100px;" value="delete"/></div>';
			}
		}

		$output.= '<input id="sidebars" type="hidden" value="' . $default . '" name="' . $value['id'] . '" />';
